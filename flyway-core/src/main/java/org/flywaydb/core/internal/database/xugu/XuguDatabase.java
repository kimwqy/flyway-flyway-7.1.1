...
import org.flywaydb.core.internal.database.oracle.OracleConnection;
...
public class XuguDatabase extends Database<XuguConnection> {
    ...
    private static final Log LOG = LogFactory.getLog(XuguDatabase.class);
    ...
    public XuguDatabase(Configuration configuration, JdbcConnectionFactory jdbcConnectionFactory, StatementInterceptor statementInterceptor) {
        ...
        pxcStrict = isXugu();
        gtidConsistencyEnforced = isXugu() ;
    }
    boolean isXugu() {
        return databaseType instanceof XuguDatabaseType;
    }
    ...
    protected String doGetCatalog() throws SQLException {
        ...
        return getMainConnection().getJdbcTemplate().queryForString("SELECT CURRENT_DATABASE() ");
    }
